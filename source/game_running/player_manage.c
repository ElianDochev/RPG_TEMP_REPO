    ...
    if (mo->millisec > 100) {
        for (int y = 0; y < MAX_Y; y++) {
            for (int x = 0; x < MAX_X; x++) {
                if (player->move == 1 && player->direction == 0 && player->map[y][x] == 'B' && player->map[y][x - 1] == ' ') {
                    player->map[y][x] = ' ';
                    player->map[y][x - 1] = 'B';
                    return 1;
                }
                if (player->move == 1 && player->direction == 1 && player->map[y][x] == 'B' && player->map[y][x + 1] == ' ') {
                    player->map[y][x] = ' ';
                    player->map[y][x + 1] = 'B';
                    return 1;
                }
                if (player->move == 1 && player->direction == 2 && player->map[y][x] == 'B' && player->map[y - 1][x] == ' ') {
                    player->map[y][x] = ' ';
                    player->map[y - 1][x] = 'B';
                    return 1;
                }
                if (player->move == 1 && player->direction == 3 && player->map[y][x] == 'B' && player->map[y + 1][x] == ' ') {
                    player->map[y][x] = ' ';
                    player->map[y + 1][x] = 'B';
                    return 1;
                }
