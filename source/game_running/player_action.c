...
static int loop_player_action(player_t *player, map_t *map, ennemi_t *ennemi, int y)
{
    for (int x = 0; x < MAX_X; ++x) {
        ...
        if (player->map[y][x] == 'E' && is_around_ennemi(player, ennemi, y, x, 'B')
        == ennemi->direction && player->attack == 0) {
            player->life--;
            return 3;
        }
    }
